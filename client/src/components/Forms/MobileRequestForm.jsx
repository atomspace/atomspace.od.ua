...
export default function MobileRequestForm(props) {
  const { headerText, formBlocks, formRegister, createOrder, isLoading } = props;
  ...
  return (
    <div className="mobile-form-request">
      ...
      {isMobile ? (
        ...
        <>
          {formRegister}
          <Button className={classname('button-next-preview')} isLoading={isLoading} onClick={createOrder}>
            {headerText}
          </Button>
        </>
      )}
